...
class ProfileProjectTab extends React.Component {
  ...
  renderProjectsTab() {
    ...
    if (this.props.myProfile) {
      prompt = (
        ...
        >
          <p className="h4-heading mb-4">
            If it might be useful to someone in our network, share it here.
          </p>
        </HintMessage>
      );
    }
    ...
    if (
      this.state.entries.published.length === 0 &&
      this.state.entries.created.length === 0
    ) {
    ...
    return (
      <div>
        {this.renderProjects(
          `Published Projects`,
          this.state.entries.published,
          prompt
        )}
        {this.renderProjects(
          `Created Projects`,
          this.state.entries.created,
          prompt
        )}
      </div>
    );
  ...
  renderTab() {
    return this.props.tabName === `favs`
      ? this.renderFavsTab()
